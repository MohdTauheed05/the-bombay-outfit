...
export function resolveCollection(slug: string): { label: string; items: Product[] } | null {
  ...
  if (match === 'New In') {
    return { label: match, items: products.filter((p) => p.isNew) }
  }
  if (match === 'Sale') {
    return { label: match, items: products.filter((p) => discountPct(p.price, p.compareAt) !== null) }
  }
  if (match === 'Designers') {
    return { label: match, items: [...products].sort((a, b) => a.brand.localeCompare(b.brand)) }
  }
  return { label: match, items: products.filter((p) => p.category === match) }
}
